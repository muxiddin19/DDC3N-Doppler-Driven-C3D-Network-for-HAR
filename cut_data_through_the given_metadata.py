...
def get_start_end_frame(path):
   ...
   # with open(./meta_data_FS2.pkl", 'r') as f:    #FigureSkating
#    with open("E:/Posec3d_new/meta_data_FS1.pkl", 'r') as f:    #FigureSkating  *
        ...
        #p1 = (Path(path).parent)
        #p = str(p1).split('\\')
        p = str((Path(path).parent)).split('\\')
        for i in range(len(data)):
            sk1.append(len(set(p).intersection(set(str(Path(data[i]['full_path']).parent).split('\\')))))
            #print("Type of str(Path(data[i]['full_path']).parent).split('\\') is:", type(str(Path(data[i]['full_path']).parent).split('\\')))
        si = max(sk1)  
        print('Intersection is ', set(p).intersection(set(str(Path(data[sk1.index(max(sk1))]['full_path']).parent).split('\\'))))
        ...
        if max(sk1) < .72:
            ...
            try:
                data = json.load(f)
                start_frame1 = data[0]['image_id'].split('.')[0]
                end_frame1 = data[-1]['image_id'].split('.')[0]
            except:
                print('Empty file appeared: ', path)
        else:
            start_frame1 = int(data[sk1.index(max(sk1))]['frame'][0])
            end_frame1 = int(data[sk1.index(max(sk1))]['frame'][1])
    

        print('Target = ', p)
        print('Target compared with = ', Path(data[sk1.index(max(sk1))]['full_path']).parent)       
        print('Similarity is = ', max(sk1))       
 
    return start_frame1, end_frame1

def get_start_end_frame1(path):
    #with open("E:/Posec3d_new/meta_data_KF1.pkl", 'rb') as f: #CrossFit *
    with open("./meta_data_KF1.pkl", 'rb') as f: #CrossFit
...
def cut_json(path0, s, e):
    ...
    try:
        ...
        pass #print('Empty file appeared: ', path)
...
for folder in folders:
    json_path = "data/CrossFit1/"+folder+"/"    #Krossfit
